      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col w-full md:w-2/3 lg:w-3/5 xl:w-1/2 bg-black border-r border-gray-800 p-8 gap-6">
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            {activeTimer ? (
              <>
                <div className="text-8xl md:text-9xl font-mono font-extrabold text-white tracking-tight mb-4 select-none">
                  {Math.floor(mainTimer.time / 60)}:{(mainTimer.time % 60).toString().padStart(2, '0')}
                </div>
                <div className="flex items-center gap-2 mb-6">
                  <span className="text-lg font-semibold text-gray-300">{activeTimer.name}</span>
                  <span className={`text-xs px-2 py-1 rounded ${mainTimer.isRunning ? 'bg-green-700 text-green-300' : 'bg-gray-700 text-gray-300'}`}>{mainTimer.isRunning ? 'Running' : 'Paused'}</span>
                </div>
                {/* Timer Controls */}
                <div className="flex items-center gap-3 mb-4">
                  <button onClick={mainTimer.start} className="p-3 bg-green-700 hover:bg-green-800 border border-green-600 rounded text-white"><Play size={22} /></button>
                  <button onClick={mainTimer.pause} className="p-3 bg-orange-700 hover:bg-orange-800 border border-orange-600 rounded text-white"><Pause size={22} /></button>
                  <button onClick={mainTimer.reset} className="p-3 bg-blue-700 hover:bg-blue-800 border border-blue-600 rounded text-white"><RotateCcw size={22} /></button>
                  <button onClick={mainTimer.stop} className="p-3 bg-red-700 hover:bg-red-800 border border-red-600 rounded text-white"><Square size={22} /></button>
                </div>
                {/* Add/Subtract Time */}
                <div className="flex items-center gap-2 mb-6">
                  <button className="px-4 py-2 bg-gray-800 border border-gray-700 rounded hover:bg-gray-700 text-sm">-1m</button>
                  <button className="px-4 py-2 bg-gray-800 border border-gray-700 rounded hover:bg-gray-700 text-sm">-10s</button>
                  <button className="px-4 py-2 bg-gray-800 border border-gray-700 rounded hover:bg-gray-700 text-sm">+10s</button>
                  <button className="px-4 py-2 bg-gray-800 border border-gray-700 rounded hover:bg-gray-700 text-sm">+1m</button>
                </div>
              </>
